  ...
  <div class="flex h-screen overflow-hidden bg-gray-100 relative">
    ...
    <aside class="w-52 min-w-52 bg-gray-50 border-r border-gray-200 flex flex-col">
      <div class="px-4 py-4 border-b border-gray-200">
        <RouterLink to="/" class="text-lg font-bold text-purple-600">ClearDrobe</RouterLink>
      </div>

      <nav class="flex flex-col py-2 flex-1">
        <p class="px-4 pt-3 pb-1 text-xs text-gray-400 uppercase tracking-wider">Principal</p>
        <RouterLink to="/dashboard" end class="nav-item" active-class="nav-active"
          >Estadísticas</RouterLink
        >
        <RouterLink to="/dashboard/armario" class="nav-item" active-class="nav-active"
          >Mi armario</RouterLink
        >
        <RouterLink to="/dashboard/atuendos" class="nav-item" active-class="nav-active"
          >Atuendos</RouterLink
        >
        <RouterLink to="/dashboard/calendario" class="nav-item" active-class="nav-active"
          >Calendario</RouterLink
        >

        <p class="px-4 pt-4 pb-1 text-xs text-gray-400 uppercase tracking-wider">Prendas</p>
        <RouterLink to="/dashboard/mas-usadas" class="nav-item" active-class="nav-active"
          >Más usadas</RouterLink
        >
        <RouterLink to="/dashboard/menos-usadas" class="nav-item" active-class="nav-active"
          >Menos usadas</RouterLink
        >

        <!-- Botón agregar en sidebar -->
        <div class="px-3 mt-4">
          <button
            @click="ui.openClothingModal()"
